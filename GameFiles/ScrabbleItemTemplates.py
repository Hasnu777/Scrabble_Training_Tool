...
# Dictionary with each letter in English Scrabble as the key, and a tuple of the score and quantity as the value IF a
# value is stored
EnglishLetters = {letter: (score, quantity) for letter, score, quantity
				in zip(AllScrabbleLetters, EnglishScore, EnglishQuantities)
				if (score is not None and quantity is not None)}
...
# Dictionary with each letter in French Scrabble as the key, and a tuple of the score and quantity as the value IF a
# value is stored
FrenchLetters = {letter: (score, quantity) for letter, score, quantity
				in zip(AllScrabbleLetters, FrenchScore, FrenchQuantities)
				if (score is not None and quantity is not None)}
...
# Dictionary with each letter in Spanish Scrabble as the key, and a tuple of the score and quantity as the value IF a
# value is stored
SpanishLetters = {letter: (score, quantity) for letter, score, quantity
				in zip(AllScrabbleLetters, SpanishScore, SpanishQuantities)
				if (score is not None and quantity is not None)}
...
SpecialLocations = {
			...
			'DW': [(1, 1), (2, 2), (3, 3), (4, 4), (4, 10), (3, 11), (2, 12), (1, 13), (7, 7), (13, 1), (12, 2),
					(11, 3), (10, 4), (10, 10), (11, 11), (12, 12), (13, 13)],
...
class Board(pg.sprite.Sprite):
	...
	# Updates a square in the board array to hold a letter
	...
	# Adds the sprite to the board's group, so it can be displayed on the game window
	def addToGroup(self, sprite):
		self.__group.add(sprite)

	# Removes the sprite from the board's group
	...
	# Gets the type of square on the board (called for squares that do not hold a tile)
...
class Rack(pg.sprite.Sprite):
	...
	def fillRack(self, tileBag):
		...
		while '' in self.__contents and not tileBag.isEmpty():
			...
			print(tileBag.bag, 'tile bag being used in the while loop')
			...
			print(letter, 'random letter selected to put into rack')
			indexPosition = tileBag.bag.index(letter)  # Takes the index position of the letter in the bag array
			print(indexPosition, 'index position of this letter in tile bag')
			if tileBag.bag[indexPosition][2] > 0:  # Checks if there are any available tiles left for that letter
				...
				if letter[2] - 1 == 0:  # Checks if all tiles for that letter are gone from the tile bag
					print(tileBag.bag, 'tile bag in use when updating isEmpty() flag')
...
class Square:
	def __init__(self, coordinates, text=' '):
		...
		self.__font = pg.font.Font(os.path.join(os.path.dirname(__file__), '../assets\\Helvetica-Font\\Helvetica.ttf'),
									16)
