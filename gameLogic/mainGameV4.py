...
def LoadGame(file):
	...
	GameBoard = ScrabbleItemTemplatesV2.Board((400, 10))  # Loading the Board object
	...
	TileBag = ScrabbleItemTemplatesV2.TileBag((50, 250), language)  # Loading TileBag object
	...
	for row in range(15):
		for column in range(15):
			...
			if GameBoard.getBoard()[row][column] in TileBag.alphabet:
				...
				tile = ScrabbleItemTemplatesV2.Tile(f'{language}Letters\\TILE_{GameBoard.getBoard()[row][column]}.png',
...
def SwapPlayers(Player1, Player2, P1Name, P2Name, P1_ID, P2_ID):  # TODO: CHECK IF THIS WORKS, PRINT INFO BEFORE AND AFTER
...
def SelectSquare(board, mouse_pos):
	for i in range(225):
		try:
			...
			square: ScrabbleItemTemplatesV2.Square = board.squares[i // 15][i % 15]
...
def PlaceTile(GameBoard, row, column, tilesPlaced):
	...
	specialLocation = GameBoard.CheckForSpecialLocation(row, column)  # Retrieving the square type
	...
	tilesPlaced[-1] = (tilesPlaced[-1][0], tile, specialLocation, square, row, column)  # Filling the stack entry for tilesPlaced
	return GameBoard, tilesPlaced


# Exchanging the selected tile
def ExchangeTile(Player, TileBag, tilesPlaced):
	...
	# tileToExchange = tilesPlaced[-1][1]  # Getting the tile from the stack
	# tileToExchangePosition = tilesPlaced[-1][0]  # Getting the tile's rack position from the stack
	if tileToExchange:
...
def CreateGameWindow(adminID='1', P1Name='', P2Name='', NewGameLang=None, FileToLoad=None):
	...
	# BUTTONS
	DetermineOrder_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((84, 725), (100, 50)), text='Pick Tile', manager=UIManager)

	ShuffleBag_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((218, 650), (100, 50)), text='Shuffle', manager=UIManager)  # Commented some code in the UIButton class to prevent pygame.USEREVENT usage
	ShuffleBag_Button.disable()

	FillRack_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((84, 650), (100, 50)), text='Fill Rack', manager=UIManager)  # Need to mention ui_button has been edited in the NEA doc
	FillRack_Button.disable()

	SwapTurn_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((218, 725), (100, 50)), text='Swap', manager=UIManager)
	SwapTurn_Button.disable()

	UndoMove_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((84, 800), (100, 50)), text='Undo Move', manager=UIManager)
	UndoMove_Button.disable()

	ExchangeTile_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((218, 800), (100, 50)), text='Exchange', manager=UIManager)
	ExchangeTile_Button.disable()

	Pause_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((1480, 20), (100, 50)), text='Pause', manager=UIManager)
	...
	GetAdminPassword_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((1460, 425), (75, 30)),  text='Enter', manager=UIManager, text_kwargs={'size': '4'})
	GetAdminPassword_Button.hide()
	GetAdminPassword_Button.disable()

	GetFileName_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((1460, 325), (75, 30)), text='Enter', manager=UIManager, text_kwargs={'size': '4'})
	GetFileName_Button.hide()
	GetFileName_Button.disable()

	CancelCloseWindow_Button = pg_gui.elements.UIButton(relative_rect=pg.Rect((1300, 475), (100, 50)), text='Cancel', manager=UIManager)
	CancelCloseWindow_Button.hide()
	CancelCloseWindow_Button.disable()

	# LABELS & OTHER GUI ITEMS
	AdminPassword_Entry = pg_gui.elements.ui_text_entry_line.UITextEntryLine(relative_rect=pg.Rect((1250, 425), (200, 30)), manager=UIManager)
	AdminPassword_Entry.hide()
	AdminPassword_Entry.disable()

	AdminPassword_Entry_Label = ScrabbleItemTemplatesV2.Text((1252, 400), (200, 50), 'Enter Admin Password:')

	FileName_Entry = pg_gui.elements.ui_text_entry_line.UITextEntryLine(relative_rect=pg.Rect((1250, 325), (200, 30)), manager=UIManager)
	FileName_Entry.hide()
	FileName_Entry.disable()

	FileName_Entry_Label = ScrabbleItemTemplatesV2.Text((1252, 300), (200, 50), 'Enter File Name (min. 6 characters):')

	InvalidPlay_Warning = ScrabbleItemTemplatesV2.Text((1210, 338), (200, 100), 'Invalid Play. Try again.')

	PickLetter_Menu = pg_gui.elements.ui_drop_down_menu.UIDropDownMenu(options_list=TileBag.alphabet[1:], starting_option='A', relative_rect=pg.Rect((350, 338), (50, 20)), manager=UIManager)
	...
	while running:
		...
		for event in pg.event.get():
			...
			if event.type == pg_gui.UI_BUTTON_PRESSED:
				...
				if event.ui_element == ExchangeTile_Button and (Player1_TileClicked or Player2_TileClicked) and not (GameOver or Paused):
					BlankTileSelected = False
					BlankTileDesignationRequired = False
					PickLetter_Menu.hide()
					if Player1_TileClicked:
						Player1, TileBag, Player1_TileClicked = ExchangeTile(Player1, TileBag, tilesPlaced)
					else:
						Player2, TileBag, Player2_TileClicked = ExchangeTile(Player2, TileBag, tilesPlaced)
				...
				if event.ui_element == DetermineOrder_Button and not GameOver:
					...
					if TileBag.alphabet.index(Player1Tile) > TileBag.alphabet.index(Player2Tile):
						Player1, Player2, P1Name, P2Name, Player1_ID, Player2_ID = SwapPlayers(Player1, Player2, P1Name, P2Name, Player1_ID, Player2_ID)
					...
					if not ExchangeOccurring:
						...
						if validPlay and wordsFormed:
							...
							if Player1_Turn:
								...
								RecordMoveToGameHistory(gameID, MoveNumber, Player2_ID, '', 0, ExchangeOccurring, False)
							else:
								...
								RecordMoveToGameHistory(gameID, MoveNumber, Player1_ID, '', 0, ExchangeOccurring, False)
			...
			if event.type == pg_gui.UI_DROP_DOWN_MENU_CHANGED and not Paused:
				if event.ui_element == PickLetter_Menu:
					...
					GameBoard, tilesPlaced, BlankTileSelected = DesignateBlank(GameBoard, tilesPlaced, TileBag.getLanguage(), event.text)
					BlankTilesInPlay -= 1
					BlankTileSelected = False
					BlankTileDesignationRequired = False
			...
			if event.type == pg.MOUSEBUTTONDOWN and pg.mouse.get_pressed()[0] and not (GameOver or Paused):
				...
				if Player1_Turn and not Player1_TileClicked and (584 <= mouse_pos[0] <= 1016) and (798 <= mouse_pos[1] <= 846) and not BlankTileSelected:
				...
				elif Player1_Turn and Player1_TileClicked and (448 <= mouse_pos[0] <= 1152) and (58 <= mouse_pos[1] <= 762) and not ExchangeOccurring:
					print('Player 1 clicked on the board @ ', mouse_pos)
					row, column = SelectSquare(GameBoard, mouse_pos)
					if not (row == -1 and column == -1):
						print(row, column, 'square selected by Player 1')
						Player1_TileClicked, tilesPlaced = GetSelectedTile(Player1, tilesPlaced)
						print(Player1_TileClicked, tilesPlaced, 'stack for Player 1')
						GameBoard, tilesPlaced = PlaceTile(GameBoard, row, column, tilesPlaced)
						print(GameBoard.getBoard(), tilesPlaced, 'board and stack for Player 1')
				...
				elif not Player1_Turn and not Player2_TileClicked and (584 <= mouse_pos[0] <= 1016) and (798 <= mouse_pos[1] <= 846) and not BlankTileSelected:
					print('Player 2 has clicked on a tile @ ', mouse_pos)
				...
				elif not Player1_Turn and Player2_TileClicked and (448 <= mouse_pos[0] <= 1152) and (58 <= mouse_pos[1] <= 762) and not ExchangeOccurring:
					row, column = SelectSquare(GameBoard, mouse_pos)
					if not (row == -1 and column == -1):
						print(row, column, 'square selected by Player 2')
						Player2_TileClicked, tilesPlaced = GetSelectedTile(Player2, tilesPlaced)
						print(Player2_TileClicked, tilesPlaced, 'stack for Player 2')
						GameBoard, tilesPlaced = PlaceTile(GameBoard, row, column, tilesPlaced)
						print(GameBoard.getBoard(), tilesPlaced, 'board and stack for Player 2')
						if BlankTileSelected:
							BlankTileDesignationRequired = True
							print(BlankTileDesignationRequired, 'blank tile designation required')
		...
		if Player1_Turn and not GameOver:
			...
			GameWindow.blit(ScrabbleItemTemplatesV2.Text((568, 875), (200, 100), f'{Player1.name} - Player 1').text,
							(568, 875))
			...
			GameWindow.blit(ScrabbleItemTemplatesV2.Text((568, 875), (200, 100), f'{Player2.name} - Player 2').text,
							(568, 875))
		...
		if RevealOtherRack and Player1_Turn:
			...
			GameWindow.blit(ScrabbleItemTemplatesV2.Text((1068, 875), (200, 100),
														f'{Player2.name} - Player 2').text, (1068, 875))
			GameWindow.blit(ScrabbleItemTemplatesV2.Text((568, 875), (200, 100),
			...
			GameWindow.blit(
				ScrabbleItemTemplatesV2.Text((1068, 875), (200, 100),
